...
import { BOARD_COUNT } from '../../lib/laneConstants.js';
...
// --- 볼러 · 볼 스펙: hand, speed/rev presets, release axis + ball numbers ----
function BallSpecSection({ play, onPlayChange }) {
...
// --- 내 라인: laydown/target boards, readout, playback controls -------------
function MyLineSection({
  play,
  onPlayChange,
  sim,
  playing,
...
// --- 오일 트랜지션: shot count → breakdown/carrydown --------------------------
function TransitionSection({ play, onShotsChange, onResetTransition }) {
  return (
    <Section
      title="오일 트랜지션"
      action={
...
// --- 추천 라인: outside (윗장) / inside (아랫장) picks ------------------------
function RecommendSection({ recs, onApplyLine }) {
...
export default function PlayTab({
  ...
  return (
    <div className="pb-4">
      <BallSpecSection play={play} onPlayChange={onPlayChange} />
      ...
      <TransitionSection play={play} onShotsChange={onShotsChange} onResetTransition={onResetTransition} />

      <RecommendSection recs={recs} onApplyLine={onApplyLine} />
    </div>
  );
}
